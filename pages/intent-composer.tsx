...
export default function IntentComposer() {
  ...
  const generatePlan = (): string[] => {
    ...
    switch (intentType) {
      case "swap":
        steps = [`${chainPrefix} Initiate swap for ${token}`, `${chainPrefix} Approve ${token} (🔒 ZK)`, `${chainPrefix} Execute swap`];
        break;
      case "transfer":
        steps = [`${chainPrefix} Verify recipient address (🔒 ZK)`, `${chainPrefix} Transfer ${token}`, `${chainPrefix} Confirm transaction (🔒 ZK)`];
        break;
      case "stake":
        steps = [`${chainPrefix} Check staking pool`, `${chainPrefix} Stake ${token} (🔒 ZK)`, `${chainPrefix} Wait for confirmation`];
        break;
      case "provide_liquidity":
        steps = [`${chainPrefix} Check pool liquidity`, `${chainPrefix} Deposit ${token} (🔒 ZK)`, `${chainPrefix} Receive LP tokens`];
        break;
      case "mint_nft":
        steps = [`${chainPrefix} Verify NFT contract (🔒 ZK)`, `${chainPrefix} Mint ${token} NFTs`, `${chainPrefix} Confirm mint`];
        break;
      default:
        steps = [];
    }
    ...
    const runSteps = () => {
      ...
      setTimeout(() => {
        setDisplayedPlan((prev) =>
          prev.map((s, idx) =>
            idx === i ? { ...s, status: isZKStep ? "zk" : "done" } : s
          )
        );
        i++;
        runSteps();
          ...
          {displayedPlan.map((item, idx) => (
            ...
            >
              ...
              <span className="flex-1">
                {item.status === "zk" ? "*** (ZK Verified)" : item.step}
              </span>
